...
export default class GameLogic {
  ...
  public updateTimers(timer: Timer) {
  ...
  public updateTimersPlus(timer: Timer) {
    timer.timeChangeCounter += globals.deltaTime;
    if (timer.timeChangeCounter > timer.timeChangeValue) {
      timer.value++;
      timer.timeChangeCounter = 0;
  ...
  public updateLevelTimer() {
    this.updateTimers(globals.levelTimer);
  }

  public updateShootTimer() {
  ...
  public updateDamagePotionTimer() {
  ...
  public updateArcherProyectile(sprite: ArcherProyectile) {
    switch (sprite.state) {
  ...
  public calculateCollisionWithBorders(sprite: Sprite) {
    if (
      sprite.xPos + sprite.imageSet.xSize > globals.canvas!.width ||
  ...
  public updateArcherProyectileLeft(sprite: ArcherProyectile) {
    switch (sprite.state) {
      case State.ARCHER_PROYECTILE_HORIZONTAL:
        sprite.physics.vx = -sprite.physics.vLimit;
        sprite.physics.vy = 0;
  ...
  public calculateCollisionSpriteAndRemove(sprite: Sprite) {
  ...
  public updateNewGame(sprite: Sprite) {
  ...
  public updateAnimationFrame(sprite: Sprite) {
  ...
  public updateSprite(sprite: Sprite) {
    ...
    switch (type) {
      ...
      case SpriteID.ARCHER_PROYECTILE:
        this.updateArcherProyectile(sprite as ArcherProyectile);
        break;
      case SpriteID.ARCHER_PROYECTILE_LEFT:
        this.updateArcherProyectileLeft(sprite as ArcherProyectile);
        break;
      default:
  ...
  public generateEnemiesRandomly() {
  ...
  public updateParticle(particle: Particle) {
    switch (particle.id) {
